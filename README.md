...
- **AI-досье** персонажа (Gemini через бэкенд) — по кнопке, с кэшем на сервере.
- **3D-сцена** на главной: Рик и Морти у портала (модели сгенерированы в Meshy AI) на подсвеченном полу с контактными тенями, с мягким парением, «дыханием»
  света из портала и вращением по drag; грузится лениво только в браузере и уважает `prefers-reduced-motion`.
...
| **three.js + @react-three/fiber + drei** | Декларативная 3D-сцена в React: `useGLTF` с meshopt-декодером, `PresentationControls`, `Float`, `useProgress` для лоадера. |
...
src/components/
  ...
  three/                      HeroScene (Canvas, композиция, лоадер) + HeroSceneLazy (dynamic, ssr: false)
...
- **3D-модели ужаты в 78 раз**: исходные экспорты Meshy (80–130 МБ каждый, 1–1,6 млн вершин, текстуры 8K) прогнаны
  через `gltf-transform`: `simplify --ratio 0.05` → `resize 1024` → `webp` → `meshopt`. Итог — 1,2–1,6 МБ на модель
  (`public/models/*.glb`), исходники лежат в gitignored `public/3d-models/`. three.js-бандл и модели грузятся только на
  главной и только в браузере (`next/dynamic`, `ssr: false`), пока они грузятся — скелетон с процентами.
...
- **3D-сцена — это +~1 МБ JS и ~4 МБ моделей на главной.** Она не блокирует рендер (грузится после текста и поиска),
  но на медленной сети появится с задержкой; при `prefers-reduced-motion` анимация выключена, рендер по требованию.
